...
export class SocketIOLikeSocket extends EventEmitter {
    ...
    async executeEventMiddleware(event, data) {
        ...
        const serverUseEventMiddleware = this.server.useEventMiddleware;
...
export class SocketIOLikeServer extends EventEmitter {
    ...
    registerUser(socket) {
        ...
        if (this.users.has(socket.id)) {
            defaultLogger.warn(`Intento de registrar usuario con ID duplicado: ${socket.id}. Desregistrando usuario anterior.`, {});
        ...
        defaultLogger.info(`Usuario registrado: ${socket.id}. Total usuarios: ${this.users.size}`, {});
    ...
    unregisterUser(socketId) {
        ...
        if (user) {
            ...
            defaultLogger.info(`Usuario desregistrado: ${socketId}. Total usuarios: ${this.users.size}`, {});
        }
    }
    // Añadir a sala
