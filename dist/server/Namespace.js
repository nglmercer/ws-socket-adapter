...
export class Namespace extends EventEmitter {
    ...
    async addSocket(socket) {
        try {
            ...
            if (this.sockets.has(socket.id)) {
                defaultLogger.warn(`Socket con ID duplicado ${socket.id} en namespace ${this.name}. Removiendo socket anterior.`, {});
            ...
            defaultLogger.error(`Error adding socket ${socket.id} to namespace ${this.name}:`, error);
    ...
    addToRoom(room, socketId) {
        ...
        defaultLogger.info(`Socket ${socketId} added to room ${room} in namespace ${this.name}`, {});
    }
    ...
    removeFromRoom(room, socketId) {
        ...
        if (roomSockets) {
            ...
            defaultLogger.info(`Socket ${socketId} removed from room ${room} in namespace ${this.name}`, {});
        }
    }
