 ...
 * Error types
 */
type ErrorType = 'TransportError' | 'CallbackError' | 'MessageError' | 'ReconnectionError';
/**
 * Disconnect reasons
 */
type DisconnectReason = 'transport close' | 'transport error' | 'server disconnect' | 'client disconnect' | 'ping timeout' | 'parse error' | 'forced close' | 'Normal closure' | 'Going away' | 'Protocol error' | 'Unsupported data' | 'No status received' | 'Abnormal closure' | 'Invalid frame payload data' | 'Policy violation' | 'Message too big' | 'Mandatory extension' | 'Internal server error' | 'TLS handshake failure';
/**
 ...
 */
type ServerMiddleware<T = any> = (socket: SocketIOSocket<ServerToClientEvents, ClientToServerEvents, InterServerEvents, T>, next: (err?: Error) => void) => void;
/**
 * Middleware function type for namespace
 */
type NamespaceMiddleware<T = any> = (socket: SocketIOSocket<ServerToClientEvents, ClientToServerEvents, InterServerEvents, T>, next: (err?: Error) => void) => void;
/**
 * Event middleware function type
 */
type EventMiddleware = (event: string, args: any[], next: (err?: Error) => void) => void;
/**
 * Generic Socket.IO compatible socket interface
 */
interface SocketIOSocket<ServerToClientEvents extends EventMap = any, ClientToServerEvents extends EventMap = any, InterServerEvents extends EventMap = any, SocketData = any> extends TypedEventEmitter<ServerToClientEvents> {
    /**
     ...
     */
    emit<K extends keyof ServerToClientEvents>(event: K, ...args: Parameters<ServerToClientEvents[K]>): boolean;
    /**
     * Emit an event with acknowledgment
     */
    emitWithAck<K extends keyof ServerToClientEvents>(event: K, ...args: Parameters<ServerToClientEvents[K]>): Promise<any>;
    /**
     * Join a room
     */
 ...
 */
interface SocketIOClient<ServerToClientEvents extends EventMap = any, ClientToServerEvents extends EventMap = any> {
    /**
     * Unique socket identifier
     */
    readonly id: string;
     ...
     */
    on<K extends keyof ServerToClientEvents>(event: K, listener: ServerToClientEvents[K]): this;
    once<K extends keyof ServerToClientEvents>(event: K, listener: ServerToClientEvents[K]): this;
    off<K extends keyof ServerToClientEvents>(event: K, listener?: ServerToClientEvents[K]): this;
    /**
     ...
     */
    emit<K extends keyof ClientToServerEvents>(event: K, ...args: [...Parameters<ClientToServerEvents[K]>, AckCallback]): this;
    /**
...
export type { EventMap, TypedEventEmitter, ServerToClientEvents, ClientToServerEvents, InterServerEvents, SocketData, ConnectionState, TransportType, ErrorType, DisconnectReason, SocketIOError, DisconnectDetails, AckCallback, EventCallback, AnyEventCallback, ClientOptions, ExtendedClientOptions, ServerOptions, ExtendedServerOptions, ServerMiddleware, NamespaceMiddleware, EventMiddleware, SocketIOSocket, SocketIOClient, BroadcastOperator, ISocket, User, Room, CustomSocket, };
export { type ConnectionStateUnion, type ErrorTypeUnion, type DisconnectReasonUnion, type TransportTypeUnion, type EnhancedSocketIOError, type ConnectionError, type CallbackError, type MessageError, type AuthError, type EnhancedServerMiddleware, type EnhancedNamespaceMiddleware, type EnhancedEventMiddleware, type EnhancedAckCallback, type EnhancedEventCallback, type EnhancedAnyEventCallback, type EnhancedConnectionState, type EnhancedServerState, type NamespaceState, type RoomState, } from './types/enhanced.js';
